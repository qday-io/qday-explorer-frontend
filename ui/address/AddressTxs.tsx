...
const AddressTxs = ({
  ...
  const addressTxsQuery = useQueryWithPages({
    ...
    sorting: getSortParamsFromValue<TransactionsSortingValue, TransactionsSortingField, TransactionsSorting["order"]>(
      sort
    ),
    scrollRef,
    options: {
  ...
  const handleNewSocketMessage: SocketMessage.AddressTxs["handler"] = React.useCallback(
    (payload) => {
      setSocketAlert("");
      ...
      queryClient.setQueryData(
        ...
        (prevData: AddressTransactionsResponse | undefined) => {
          ...
          return {
            ...prevData,
            items: [...newItems, ...prevData.items].sort(sortTxsFromSocket(sort)),
          };
        }
      );
    },
