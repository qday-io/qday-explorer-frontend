              ...
              <Flex mb={3} flexWrap="wrap" lineHeight="30px">
                ...
                <Skeleton isLoaded={!isPlaceholderData} mr={3}>
                  <Text
                    variant="secondary"
                    whiteSpace="pre"
                  >{` - ${Number(item.amount).toLocaleString()} item${Number(item.amount) > 1 ? "s" : ""}`}</Text>
                </Skeleton>
              ...
              >
                {item.token_instances.map((instance, index) => {
                  const key =
                    item.token.address +
                    "_" +
                    (instance.id && !isPlaceholderData ? `id_${instance.id}` : `index_${index}`);

                  return <NFTItem key={key} {...instance} token={item.token} isLoading={isPlaceholderData} />;
                })}
                {hasOverload && (
