...
const ContractCode = ({ addressHash, contractQuery, channel }: Props) => {
  ...
  const constructorArgs = (() => {
    ...
    const decoded = data.decoded_constructor_args.map(([value, { name, type }], index) => {
      const valueEl =
        type === "address" ? (
          <AddressEntity address={{ hash: value }} noIcon display="inline-flex" maxW="100%" />
        ) : (
          <span>{value}</span>
        );
      return (
    ...
    <>
      <Flex flexDir="column" rowGap={2} mb={6} _empty={{ display: "none" }}>
        {data?.is_blueprint && (
          <Box>
            ...
            <LinkExternal href="https://eips.ethereum.org/EIPS/eip-5202">ERC-5202 Blueprint contract</LinkExternal>
          </Box>
        )}
        {data?.is_verified && (
        ...
        {(data?.is_changed_bytecode || isChangedBytecodeSocket) && (
          <Alert status="warning">
            Warning! Contract bytecode has been changed and does not match the verified one. Therefore, interaction with
            this smart contract may be risky.
          </Alert>
        )}
        {!data?.is_verified && data?.verified_twin_address_hash && !data?.minimal_proxy_address_hash && (
          <Alert status="warning" whiteSpace="pre-wrap" flexWrap="wrap">
            ...
            />
            <chakra.span mt={1}>
              All functions displayed below are from ABI of that contract. In order to verify current contract, proceed
              with{" "}
            </chakra.span>
            <LinkInternal
              href={route({ pathname: "/address/[hash]/contract-verification", query: { hash: addressHash || "" } })}
      ...
      <Flex flexDir="column" rowGap={6}>
        ...
        {data?.source_code && addressHash && (
          <ContractSourceCode address={addressHash} implementations={addressInfo?.implementations || undefined} />
        )}
            ...
            beforeSlot={
              data.is_self_destructed ? (
                <Alert status="info" whiteSpace="pre-wrap" mb={3}>
                  Contracts that self destruct in their constructors have no contract code published and cannot be
                  verified. Displaying the init data provided of the creating transaction.
                </Alert>
              ) : null
            }
            textareaMaxHeight="200px"
