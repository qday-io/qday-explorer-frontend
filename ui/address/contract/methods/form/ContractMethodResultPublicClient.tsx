...
const ContractMethodResultPublicClient = ({ data, abiItem, onSettle, mode: modeProps, containerProps }: Props) => {
    ...
    <>
      ...
      >
        {abiItem.outputs.map((output, index) => (
          <ResultItem key={index} abiParameter={output} data={isError ? undefined : formattedData[index]} mode={mode} />
        ))}
      </Flex>
    </>
  );
