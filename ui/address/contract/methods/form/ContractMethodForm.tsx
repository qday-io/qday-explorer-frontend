...
const NO_WALLET_CLIENT_TEXT =
  "Blockchain interaction is not available at the moment since WalletConnect is not configured for this application. Please contact the service maintainer to make necessary changes in the service configuration.";
...
const ContractMethodForm = ({ data, attempt, onSubmit, onReset, isOpen }: Props) => {
  ...
  const onFormSubmit: SubmitHandler<ContractMethodFormFields> = React.useCallback(
    async (formData) => {
  ...
  return (
    <Box>
      <FormProvider {...formApi}>
        <chakra.form noValidate onSubmit={formApi.handleSubmit(onFormSubmit)} onChange={handleFormChange}>
      ...
      {result && result.source === "wallet_client" && (
        <ContractMethodResultWalletClient data={result.data} onSettle={handleResultSettle} />
      )}
      {"outputs" in data && data.outputs.length > 0 && (
        <ContractMethodResultPublicClient
          ...
          containerProps={{
            backgroundColor: color.filledDisabled,
