...
export const ContractSourceCode = ({ address, implementations }: Props) => {
  ...
  const heading = (
    <Skeleton isLoaded={!isLoading} fontWeight={400} fontSize={{ base: 14, md: 16 }} color={color.textPrimary}>
      <span>Contract source code</span>
      {contractQuery.data?.language && (
        <Text whiteSpace="pre" as="span" variant="secondary" textTransform="capitalize" color={color.textSecondary}>
          {" "}
          ({contractQuery.data.language})
        </Text>
      )}
