...
const CodeEditorSearch = ({
  ...
  React.useEffect(() => {
    ...
    const matches = models.map((model) =>
      model.findMatches(debouncedSearchTerm, false, isMatchRegex, isMatchCase, isMatchWholeWord ? "true" : null, false)
    );

    models.forEach((model, index) => {
      const filePath = model.uri.path;
      const prevDecorations = decorations.current[filePath] || [];
      const newDecorations: Array<monaco.editor.IModelDeltaDecoration> = matches[index].map(({ range }) => ({
        range,
  ...
  const searchResultNum = (() => {
    ...
    return (
      <Box px="8px" fontSize="13px" lineHeight="18px" mb="8px">
        {totalResults} result{totalResults > 1 ? "s" : ""} in {searchResults.length} file
    ...
    <Box>
      ...
      >
        {searchResults.map((item) => (
          <CodeEditorSearchSection key={item.file_path} data={item} onItemClick={handleResultItemClick} />
        ))}
      </Accordion>
    </Box>
  );
