...
const CodeEditor = ({ data, remappings, libraries, language, mainFile, contractName }: Props) => {
  ...
  const handleSelectFile = React.useCallback(
    (index: number, lineNumber?: number) => {
      setIndex(index);
      setTabs((prev) =>
        prev.some((item) => item === data[index].file_path) ? prev : [...prev, data[index].file_path]
      );
        ...
        if (prev.length > 1) {
          ...
          if (isActive) {
            const nextActiveIndex = data.findIndex(
              (item) => item.file_path === prev[tabIndex === 0 ? 1 : tabIndex - 1]
            );
  ...
  const renderErrorScreen = React.useCallback(() => {
    return (
      <Center bgColor={themeColors["editor.background"]} w="100%" borderRadius="md">
