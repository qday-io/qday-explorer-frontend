...
const Icon = (props: IconProps) => {
  return <EntityBase.Icon {...props} name={props.name ?? "transactions_slim"} />;
};

type ContentProps = Omit<EntityBase.ContentBaseProps, "text"> & Pick<EntityProps, "hash" | "text">;

const Content = chakra((props: ContentProps) => {
  return <EntityBase.Content {...props} text={props.text ?? props.hash} fontSize={16} lineHeight={6} />;
});
...
const TxEntity = ({ hasIcon = true, ...props }: EntityProps) => {
  const linkProps = _omit(props, ["className"]);
  const partsProps = _omit(props, ["className", "onClick"]);

  return (
    <Container className={props.className}>
      {hasIcon && <Icon {...partsProps} name={props.iconName} color={props.iconColor} />}

      <Link {...linkProps}>
        <Content {...partsProps} color={props.contentColor} />
      </Link>
      <Copy {...partsProps} />
    </Container>
  );
