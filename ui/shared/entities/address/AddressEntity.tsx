...
const Link = chakra((props: LinkProps) => {
  ...
  return (
    <EntityBase.Link {...props} href={props.href ?? defaultHref} color="red">
      {props.children}
    </EntityBase.Link>
  );
});
...
const Icon = (props: IconProps) => {
  ...
  if (props.address.is_contract) {
    ...
    const isVerified = isProxy
      ? props.address.is_verified && props.address.implementations?.every(({ name }) => Boolean(name))
      : props.address.is_verified;
    const contractIconName: EntityBase.IconBaseProps["name"] = props.address.is_verified
      ? "contracts/verified"
      : "contracts/regular";
