...
const TxDetailsTokenTransfer = ({ data }: Props) => {
  const content = (() => {
    ...
    switch (data.token.type) {
      ...
      case "ERC-721": {
        ...
        return <NftTokenTransferSnippet token={data.token} tokenId={total.token_id} value="1" />;
      }
  ...
  return (
    <Flex alignItems="flex-start" flexWrap="wrap" columnGap={2} rowGap={3} flexDir="row" w="100%" fontWeight={500}>
      <AddressFromTo from={data.from} to={data.to} truncation="constant" noIcon fontWeight="500" />
      {content}
    </Flex>
  );
};
