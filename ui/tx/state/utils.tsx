...
export function getStateElements(data: TxStateChange, isLoading?: boolean) {
  ...
  switch (data.type) {
    case "coin": {
      ...
      const changeColor = beforeBn.lte(afterBn) ? color.textGreen : color.textRed;
    ...
    case "token": {
      const tokenLink = <TokenEntity token={data.token} isLoading={isLoading} noIcon noCopy onlySymbol w="auto" />;
      ...
      const tokenId = (() => {
        if (!Array.isArray(data.change)) {
          if ("token_id" in data && data.token_id) {
            return <NftEntity hash={data.token.address} id={data.token_id} isLoading={isLoading} />;
          } else {
            return null;
          }
        }
