    ...
    >
      ...
      {rollupFeature.isEnabled &&
        ...
        !config.UI.views.block.hiddenFields?.batch && (
          <>
            <DetailsInfoItem.Label contentProps={labelContentProps} hint="Batch number" isLoading={isPlaceholderData}>
              Batch
            </DetailsInfoItem.Label>
            <DetailsInfoItem.Value contentProps={valueContentProps}>
      ...
      {!config.UI.views.block.hiddenFields?.miner && (
        <>
          ...
          <DetailsInfoItem.Value contentProps={valueContentProps}>
            <AddressEntity address={data.miner} isLoading={isPlaceholderData} colorHighlight={color.textInfo} />
          </DetailsInfoItem.Value>
        </>
      )}
      ...
      {!rollupFeature.isEnabled &&
        ...
        !config.UI.views.block.hiddenFields?.total_reward && (
          <>
            <DetailsInfoItem.Label
              contentProps={labelContentProps}
              hint={`For each block, the ${validatorTitle} is rewarded with a finite amount of ${config.chain.currency.symbol || "native token"} 
          on top of the fees paid for all transactions in the block`}
              isLoading={isPlaceholderData}
            >
              Block reward
            </DetailsInfoItem.Label>
            <DetailsInfoItem.Value columnGap={1} contentProps={valueContentProps}>
              <Skeleton isLoaded={!isPlaceholderData}>
      ...
      {!config.UI.views.block.hiddenFields?.burnt_fees && !burntFees.isEqualTo(ZERO) && (
        <>
          <DetailsInfoItem.Label
            contentProps={labelContentProps}
            hint={`Amount of ${config.chain.currency.symbol || "native token"} burned from transactions included in the block. 
      ...
      {isExpanded && !isPlaceholderData && (
        <>
          ...
          {data.bitcoin_merged_mining_header && (
            <>
              <DetailsInfoItem.Label contentProps={labelContentProps} hint="Merged-mining field: Bitcoin header">
                Bitcoin merged mining header
              </DetailsInfoItem.Label>
              <DetailsInfoItem.Value contentProps={valueContentProps} flexWrap="nowrap" alignSelf="flex-start">
          ...
          {data.bitcoin_merged_mining_coinbase_transaction && (
            <>
              <DetailsInfoItem.Label contentProps={labelContentProps} hint="Merged-mining field: Coinbase transaction">
          ...
          {data.bitcoin_merged_mining_merkle_proof && (
            <>
              <DetailsInfoItem.Label contentProps={labelContentProps} hint="Merged-mining field: Merkle proof">
          ...
          <DetailsInfoItem.Value contentProps={valueContentProps} overflow="hidden">
          ...
          <DetailsInfoItem.Label contentProps={labelContentProps} hint="The SHA256 hash of the block">
            Hash
          </DetailsInfoItem.Label>
          ...
          {rollupFeature.isEnabled && rollupFeature.type === "arbitrum" && data.arbitrum && (
            <>
              ...
              <DetailsInfoItem.Value contentProps={valueContentProps}>{data.arbitrum.send_root}</DetailsInfoItem.Value>
