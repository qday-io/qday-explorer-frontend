  ...
  return (
    <Flex position="relative" flexDirection="column" flexGrow={1}>
      ...
      >
        ...
        {hasAccount && (
          <Box as="nav" mt={3} pt={3} borderTopWidth="1px" borderColor="divider">
            <VStack as="ul" spacing="1" alignItems="flex-start">
              {accountNavItems.map((item) => (
                <NavLink key={item.text} item={item} onClick={onNavLinkClick} isCollapsed={isCollapsed} />
              ))}
            </VStack>
          </Box>
        )}
      ...
      {openedGroupIndex >= 0 && (
        <Box as={motion.nav} w="100%" mt={6} position="absolute" top={0} style={{ x: subX }} key="sub">
          <Flex alignItems="center" px={2} py={2.5} w="100%" h="50px" onClick={onGroupItemClose} mb={1}>
            <IconSvg name="arrows/east-mini" boxSize={6} mr={2} />
            ...
            {isGroupItem(openedItem) &&
              openedItem.subItems?.map((item, index) =>
                Array.isArray(item) ? (
                  ...
                  >
                    {item.map((subItem) => (
                      <NavLink key={subItem.text} item={subItem} onClick={onNavLinkClick} isCollapsed={isCollapsed} />
                    ))}
                  </Box>
                ) : (
