      ...
      >
        ...
        <Portal>
          <PopoverContent w={`${menuWidth.current}px`} ref={menuRef}>
            <PopoverBody p={0} color="chakra-body-text" top={0} margin={0}>
              <Box
                maxH="50vh"
                overflowY="auto"
                ...
                backgroundColor={color.bgPopup}
                margin={0}
                boxShadow="0px 8px 28px 0px rgba(0, 44, 119, 0.05), 0px 4px 12px 0px rgba(0, 44, 119, 0.10)"
              >
            ...
            {searchTerm.trim().length > 0 && query.data && query.data.length >= 50 && (
              <PopoverFooter>
                <LinkInternal href={route({ pathname: "/search-results", query: { q: searchTerm } })} fontSize="sm">
                  View all results
                </LinkInternal>
              </PopoverFooter>
            )}
